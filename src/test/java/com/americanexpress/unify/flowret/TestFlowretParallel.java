...
public class TestFlowretParallel {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String journey = "parallel_test";
    String json = BaseUtils.getResourceAsString(TestFlowretParallel.class, "/flowret/" + journey + ".json");

    init(dirPath);
    Rts rts = Flowret.instance().getRunTimeService(dao, factory, handler, null);

    if (new File(dirPath + "flowret_journey-2.json ").exists() == false) {
      rts.startCase("2", json, null, null);
    }
    else {
      rts.resumeCase("2");
    }
  ...
  protected static void init(String dirPath) {
    ...
    factory = new TestComponentFactoryParallel();
    handler = new TestHandler();
    Flowret.init(10, 3000, "-");
  }
